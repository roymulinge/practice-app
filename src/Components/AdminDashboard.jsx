...
export default function AdminDashboard() {
  const [page, setPage] = useState("add"); // "add" or "list"
  ...
  const loadStudents = async () => {
    try {
      ...
      const data = snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data(),
        expectedFee: Number(doc.data().expectedFee),
        feePaid: Number(doc.data().feePaid),
      }));
      setStudents(data);
    } catch (err) {
  ...
  const handleAddStudent = async (e) => {
    ...
    try {
      ...
      setMessage(`Student "${name}" added successfully!`); // feedback message
      loadStudents(); // refresh table
      setPage("list"); // switch to list page after adding
    } catch (err) {
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 bg-gray-100 p-8">
        {message && (
          <div className="max-w-md mx-auto mb-4 bg-green-100 border border-green-400 text-green-700 px-4 py-2 rounded">
            {message}
          </div>
        )}
                  ...
                  {students.map(s => (
                    <tr key={s.id} className="hover:bg-gray-100">
                      ...
                      <td className="border px-2 py-1">
                        <button onClick={() => handleDeleteStudent(s.id)} className="bg-red-500 hover:bg-red-600 text-white px-2 py-1 rounded">
                          Delete
                        </button>
                      </td>
                    </tr>
